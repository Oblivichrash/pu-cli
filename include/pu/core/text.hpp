...
constexpr bool IsContinuationByte(unsigned char byte) {
...
constexpr unsigned int DecodeCodePoint(std::string_view text, std::size_t index,
  ...
  unsigned int code_point =
      static_cast<unsigned char>(text[index]) & kLeadPayloadMask[length - 1];
  for (std::size_t offset = 1; offset < length; ++offset) {
    code_point = (code_point << 6) |
                 (static_cast<unsigned char>(text[index + offset]) &
                  kContinuationPayloadMask);
  }
  return code_point;
}

constexpr bool IsOverlong(std::size_t length, unsigned int code_point) {
  return (length == 2 && code_point < 0x80) ||
...
inline std::optional<std::size_t> ValidSequenceLength(std::string_view text,
                                                      std::size_t index) {
  ...
  if (IsOverlong(length, code_point) || IsSurrogate(code_point) ||
      code_point > kMaxCodePoint) {
    return std::nullopt;
  }
  return length;
}
