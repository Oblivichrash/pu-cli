...
std::string WrapMcpResult(const std::string& raw) {
  bool is_error = raw.rfind("Error:", 0) == 0 || raw.rfind("MCP error:", 0) == 0 ||
                  raw.rfind("MCP call error:", 0) == 0;
  return tools::MakeToolResultJson(!is_error, raw, std::string{}, is_error ? raw : std::string{},
                                   is_error ? 1 : 0);
}

}  // namespace

McpTool::McpTool(mcp::McpClient* client, const ToolDefinition& def, std::string server_name)
    : client_(client),
      def_(def),
