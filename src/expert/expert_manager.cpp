...
void ExpertManager::SetActiveExpert(const std::string& name) {
  if (experts_.count(name)) active_expert_ = name;
}
...
std::string ExpertManager::Dispatch(const std::string& input) {
  if (experts_.empty()) return "No experts available.";
  ...
  if (it == experts_.end()) {
    ...
    if (it == experts_.end()) it = experts_.begin();
  }
  if (active_expert_.empty() && input[0] != '@') active_expert_ = it->first;
...
void ExpertManager::ClearSessions() {
  for (auto& [name, expert] : experts_) expert->ResetSession();
...
void ExpertManager::RestoreExperts(
  ...
  for (const auto& [name, messages] : states) {
    ...
    if (it != experts_.end()) it->second->LoadState(messages);
  }
}
