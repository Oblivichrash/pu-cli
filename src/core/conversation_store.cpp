...
void ConversationStore::Save(const Conversation& conv) {
  ...
  if (!file.is_open()) throw StoreError("Failed to write conversation: " + conv.id);
  file << j.dump(2);
}
...
Conversation ConversationStore::Load(const std::string& id) const {
  ...
  json j;
  try { file >> j; } catch (const json::parse_error&) { throw StoreError("Invalid conversation data for: " + id); }
  Conversation conv;
